...
class RemoteApiClient:
    ...
    def ping(self):
        _,_,_,_,buff = self.client.simxCallScriptFunction('ping@b0RemoteApiServer',
                                                              sim.sim_scripttype_customizationscript,
                                                              [[],[],[],''],
                                                              self.client.simxServiceCall())
        table = buff.decode("utf-8").split(',')
        self.drones = {}
        self.rovers = {}
        for i in range(len(table)):
            if i%2 and re.match('Rover(#?)\d?(?![A-Z])', table[i]):
                self.drones[f"{len(self.drones)}"] = Quadricopter(self.client, len(self.drones)-1, self.uavctrl)
            if i%2 and re.match('Quadricopter(#?)\d?(?![A-Z])', table[i]):
                self.rovers[f"{len(self.rovers)}"] = Rover(self.client, len(self.rovers)-1, self.ugvctrl)
        self.config.population = NestedNameSpace({
            "drones": len(self.drones),
            "rovers": len(self.rovers)
        })
